...
// Since we don't have easy access to compositor includes via CMake yet without modifying paths,
// and to keep it simple, we will support PNG via Cairo first.
...
OSFWallpaper::OSFWallpaper(const std::string& imagePath) : imagePath_(imagePath) {
    ...
    surface_->setAnchor(Anchor::Top | Anchor::Bottom | Anchor::Left | Anchor::Right);

    // Exclusive zone 0 because windows can cover it
    surface_->setExclusiveZone(0); // -1 often means ignore exclusive zone logic, 0 means don't push others.
    ...
    if (!imagePath_.empty()) {
        ...
        if (cairo_surface_status(cachedImage_) != CAIRO_STATUS_SUCCESS) {
            std::cerr << "Failed to load image: " << imagePath_ << " (Only PNG is supported currently)" << std::endl;
    ...
    // Bind callbacks
    surface_->onDraw([this](cairo_t* cr, int w, int h) {
        this->draw(cr, w, h);
    });
}
