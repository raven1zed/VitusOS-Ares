...
constexpr uint32_t MarsOrange = 0xFFD4622A; // Corrected to #D4622A
...
// Neutral colors (VitusOS Light/Modern Theme)
constexpr uint32_t LunarGray = 0xFFF5F5F5; // Title bar, panels (Light Grey)
constexpr uint32_t DeepSpace = 0xFFFFFFFF; // Window backgrounds (Pure White)
constexpr uint32_t Nebula = 0xFFE0E0E0;    // Hover states (Light Gray)
constexpr uint32_t StarWhite = 0xFF1A1A1A; // Primary text (Near Black)
constexpr uint32_t MoonGray = 0xFF666666;  // Secondary text (Dark Grey)

// Traffic light buttons (VitusOS style)
constexpr uint32_t ButtonClose = 0xFFD4622A;    // Orange (close)
constexpr uint32_t ButtonMinimize = 0xFFD4A93E; // Gold (minimize)
constexpr uint32_t ButtonMaximize = 0xFF4A9FD4; // Blue (maximize)

// Transparency
constexpr uint32_t GlassBackground = 0xC0F5F5F5; // 75% opacity LunarGray
constexpr uint32_t PanelBackground = 0xF5F5F5F5; // 96% opacity LunarGray
constexpr uint32_t DockBackground = 0xEEFFFFFF;  // 93% opacity Pure White
...
constexpr int ButtonCornerRadius = 6;  // Default button radius
...
// Draw rounded rectangle path (Bottom only)
inline void roundedBottomRect(cairo_t *cr, double x, double y, double w,
                              double h, double r) {
  cairo_new_sub_path(cr);
  cairo_move_to(cr, x, y);                             // Top Left (Square)
  cairo_line_to(cr, x + w, y);                         // Top Right (Square)
